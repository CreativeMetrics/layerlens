...
- **QoL / GTM UI** (`qol-changes.content.ts` → `qol-changes.inject.ts`): runs on `tagmanager.google.com`.
  ...
  - **Da push dataLayer wizard** — pulsante "Da push datalayer…" nella toolbar della pagina Variabili. Incolla un push (anche con sintassi JS grezza, chiavi non quotate, wrapper `dataLayer.push()`). Modal con tre sezioni indipendenti: (1) crea variabili DataLayer (`publicId: 'v'`) per i percorsi dot-notation del push, (2) crea trigger Custom Event, (3) crea tag GA4 Event (`gaawe`). Le sezioni sono abilitabili singolarmente; il trigger creato viene passato al tag. Tutte le creazioni usano il pattern template: clona un'entità esistente dello stesso tipo, cancella i campi tipo-specifici, imposta i nuovi.
